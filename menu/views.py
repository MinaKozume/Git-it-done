...
def menu_page(request):
    ...
    # Build favourites info for authenticated users
    fav_product_ids = set()
    fav_map = {}
    if request.user.is_authenticated:
        fav_qs = FavouriteItem.objects.filter(user=request.user).select_related('product')
        for f in fav_qs:
            fav_product_ids.add(f.product.id)
            fav_map[f.product.id] = f.id
    ...
    return render(request, "menu/menu.html", {
        ...
        # favourites mapping for current user (product_id -> favourite_item_id)
        "favourite_product_ids": fav_product_ids,
        "fav_map": fav_map,
    })
